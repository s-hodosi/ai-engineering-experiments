...
The system SHALL parse "Posted X ago" / "Reposted X ago" text from Tavily snippets and discard results whose parsed age exceeds `FRESHNESS_MAX_AGE_DAYS` (default 3).

#### Scenario: Job posted within threshold
- **WHEN** the snippet contains "Posted 2 days ago" and `FRESHNESS_MAX_AGE_DAYS` is 3
- **THEN** the result passes the freshness filter

#### Scenario: Job posted beyond threshold
- **WHEN** the snippet contains "Posted 2 weeks ago" or "Posted 1 month ago"
- **THEN** the result SHALL be discarded without an LLM call

#### Scenario: No posting age text in snippet
- **WHEN** the snippet contains no "Posted X ago" pattern
